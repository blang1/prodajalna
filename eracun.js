...
streznik.get('/', function(zahteva, odgovor) {
  if(zahteva.session.momentalnaStranka){
...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function(greska, polja, datoteke){
    if(datoteke !=null && !greska){
      strankaIzRacuna(polja.seznamRacunov, function(greska, stranka){
        if(!greska && stranka!=null){
          pesmiIzRacuna(polja.seznamRacunov, function(greska, pesmi){
            if(pesmi!=null && !greska){
...
var getTheCustomer = function(customersId, callback){
  pb.all("SELECT * FROM Customer WHERE Customer.CustomerId = " + customersId, function(napaka, vrstice){
    callback(napaka,vrstice);
  })
}
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  pesmiIzKosarice(zahteva, function(pesmi) {
    if (!pesmi) {
      ...
      getTheCustomer(zahteva.session.momentalnaStranka, function(napaka, stranka) {
...
streznik.post('/stranka', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    zahteva.session.momentalnaStranka = parseInt(polja.seznamStrank);
    odgovor.redirect('/')
  });
})
...
streznik.post('/odjava', function(zahteva, odgovor) {
    zahteva.session.momentalnaStranka = null;
    odgovor.redirect('/prijava') 
})
